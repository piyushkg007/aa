...
COMPONENTS = [[window.button_left_1,window.button_center_1, window.button_right_1],[window.button_left_2,window.button_center_2, window.button_right_2]]
curBlinks = 0
curLvl = 0
# curTime = time.time()
...
def set_but_color(left,right):
    global curLvl
    global window
    comps = COMPONENTS[curLvl]
    
    for lvl in COMPONENTS:
        for comp in lvl:
            comp.setStyleSheet("background-color: White")

    lab = 'Blinks: ' + str(curBlinks)
    window.blink_label.setText(lab)

    if left=='CENTER' and right=='CENTER':
        comps[1].setStyleSheet("background-color: Cyan")
...
with map_face_mesh.FaceMesh(min_detection_confidence =0.45 ,min_tracking_confidence=0.45) as face_mesh:
    # starting time here 
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            set_but_color(eye_position_left,eye_position_right)
        else:
            set_but_color('','')
            pass
